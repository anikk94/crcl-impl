...
class DataThingType(object):
    ...
    def __str__(self):
        ...
        outstr = xmldec + output.getvalue().decode('utf-8')
        output.close()
        return outstr
...
class PoseAndSetType(PoseType):
    ...
    def tree(self, root=None):
        if root is None:
            root = ET.Element(None)
        super().tree(root)
        ET.SubElement(root, "Coordinated").text = "true" if self.Coordinated else "false"
        if self.TransSpeed is not None:
            self.TransSpeed.tree(root)
        if self.RotSpeed is not None:
            self.RotSpeed.tree(root)
        if self.TransAccel is not None:
            self.TransAccel.tree(root)
        if self.RotAccel is not None:
            self.RotAccel.tree(root)
        if self.Tolerance is not None:
            self.Tolerance.tree(root)
        return ET.ElementTree(root)
...
class MiddleCommandType(CRCLCommandType):
    def __init__(self, CommandID, **kwargs):
        super().__init__(CommandID, **kwargs)
...
class MoveThroughToType(MiddleCommandType):
    ...
    def tree(self, root=None):
        ...
        super().tree(el)
        ET.SubElement(el, "MoveStraight").text = "true" if self.MoveStraight else "false"
...
class CRCLProgramType(DataThingType):
    ...
    def tree(self, root=None):
        if root is None:
            root = ET.Element("CRCLProgram", attrib=progdict)
        super().tree(root)
...
def toCommandStateType(s):
    if s == CommandStateType.DONE:
        return s
    if s == CommandStateType.WORKING:
        return s
    if s == CommandStateType.TIMEOUT:
        return s
...
class JointStatusType(DataThingType):
    ...
    def tree(self, root=None):
        ...
        if self.JointPosition is not None:
            ET.SubElement(el, "JointPosition").text = str(self.JointPosition)
        if self.JointTorqueOrForce is not None:
            ET.SubElement(el, "JointTorqueOrForce").text = str(self.JointTorqueOrForce)
        if self.JointVelocity is not None:
            ET.SubElement(el, "JointVelocity").text = str(self.JointVelocity)
...
class JointStatusesType(DataThingType):
    ...
    def tree(self, root=None):
        if len(self.JointStatus) == 0:
            return ET.ElementTree(ET.Element(None))
        if root is None:
            root = ET.Element(None)
        else:
            super().tree(root)
...
class ParallelGripperStatusType(GripperStatusType):
    def __init__(self, GripperName, Separation, **kwargs):
        super().__init__(GripperName, **kwargs)
...
class VacuumGripperStatusType(GripperStatusType):
    ...
    def tree(self, root=None):
        ...
        super().tree(el)
        ET.SubElement(el, "IsPowered").text = "true" if self.IsPowered else "false"
        return ET.ElementTree(root)
...
class CRCLStatusType(DataThingType):
    ...
    def tree(self, root=None):
        if root is None:
            root = ET.Element("CRCLStatus", attrib=statdict)
        super().tree(root)
        self.CommandStatus.tree(root)
        if self.PoseStatus is not None:
            self.PoseStatus.tree(root)
        if self.JointStatuses is not None:
            self.JointStatuses.tree(root)
        if self.GripperStatus is not None:
            self.GripperStatus.tree(root)
        return ET.ElementTree(root)
    ...
    def read(self, treeStr):
        tree = ET.parse(StringIO(treeStr.rstrip(' \t\n\r\0')))
        root = tree.getroot()
        if root.tag == "CRCLStatus":
            for child in root:
                if child.tag == "CommandStatus":
                    ...
                    if t:
                        self.CommandStatus.CommandID = t
                    t = child.findtext("StatusID")
                    if t:
                        self.CommandStatus.StatusID = t
                    t = child.findtext("CommandState")
                    if t:
                        self.CommandStatus.CommandState = toCommandStateType(t)
                elif child.tag == "Pose":
                    for cc in child:
                        if cc.tag == "Point":
...
def MatrixToQuaternion(Min):
    try:
        M = MatrixType(unit(Min.X), unit(Min.Y), unit(Min.Z))
    except Exception as e:
        raise ValueError from e
