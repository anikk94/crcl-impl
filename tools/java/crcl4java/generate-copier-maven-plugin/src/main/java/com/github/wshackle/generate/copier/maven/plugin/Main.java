...
//        }
	...
	try {
		...
		if (initJarString.startsWith("~/")) {
		    ...
		    localParams.jar = new File(new File(getCurrentDir()), initJarString.substring(2))
			    .getCanonicalPath();
		} else if (initJarString.startsWith("../")) {
		    localParams.jar = new File(new File(getCurrentDir()).getParentFile(), initJarString.substring(3))
			    .getCanonicalPath();
		} else {
...
//        Set<Class<?>> excludedClasses = new TreeSet<>();
	...
	if (null != jarPath) {
		...
		for (File clssFile : filesList) {
		    ...
		    try {
			addClass(cl, className, localParams, packagesSet, classesList, excludedClassNames,
				foundClassNames);
		    } catch (ClassNotFoundException | NoClassDefFoundError ex) {
			System.err.println("Caught " + ex.getClass().getName() + ":" + ex.getMessage()
				+ " for className=" + className + ", jarPath=" + jarPath);
		    }
		...
		if (!jarFile.exists()) {
			...
			if (jarFileParent.exists()) {
			    System.out.println(
				    "jarFileParent.listFiles() = " + Arrays.toString(jarFileParent.listFiles()));
			} else {
				...
				System.out.println("jarFileGrandParent.listFiles() = "
					+ Arrays.toString(jarFileGrandParent.listFiles()));
			    }
			}
		    }
		}
		...
		for (ZipEntry entry = zip.getNextEntry(); entry != null; entry = zip.getNextEntry()) {
			...
			if (localParams.classnamesToFind != null
			    ...
			    if (verbose) {
				logString("skipping className=" + className + " because it does not found in="
					+ localParams.classnamesToFind);
			    }
			    continue;
			}
			try {
...
//        }
	...
	if (null != localParams.javacloner && localParams.javacloner.length() > 1) {
		...
		if (outFile.isDirectory()) {
			...
			throw new MojoExecutionException(
				"output directory not given: localParams.output=" + localParams.output);
		    }
		}
	    } else {
		throw new MojoExecutionException(
    ...
    private static @Nullable Path getPath(@Nullable String fname) {
    ...
    private static void addClass(URLClassLoader cl, String className, LocalParams localParams, Set<String> packagesSet,
	    List<Class<?>> classesList, Set<String> excludedClassNames, Set<String> foundClassNames)
