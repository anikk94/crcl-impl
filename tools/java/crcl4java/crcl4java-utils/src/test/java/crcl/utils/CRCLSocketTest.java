...
public class CRCLSocketTest {
	...
	static final String programAllXmlTrimmed = "<CRCLProgram\n"
	...
	private static final String MOVETHROUGHTO_XML = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
			+ "<CRCLCommandInstance>\n"
...
//	private static final String OLD_MOVETHROUGHTO_XML = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
//			+ "<CRCLCommandInstance>\n"
//			+ " <CRCLCommand xsi:type=\"MoveThroughToType\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n"
//			+ "    <CommandID>2</CommandID>\n" + "    <MoveStraight>false</MoveStraight>\n" + "    <Waypoint>\n"
//			+ "      <Point>\n" + "        <X>2.5</X> <Y>1</Y> <Z>1</Z>\n" + "      </Point>\n" + "      <XAxis>\n"
...
//			+ "    <NumPositions>2</NumPositions>\n" + "  </CRCLCommand>\n" + "</CRCLCommandInstance>";
	...
	public void testReadUntilEndTag2() throws Exception {
		...
		try (ByteArrayInputStream bais = new ByteArrayInputStream(programAllXml.getBytes())) {
			String out = CRCLSocket.readUntilEndTag("CRCLProgram", bais);
			assertEquals(programAllXmlTrimmed, out);
		}
		boolean validate = false;
		try (CRCLSocket instance = new CRCLSocket()) {
			try (ByteArrayInputStream bais = new ByteArrayInputStream(STATUS_XML.getBytes())) {
				String str = CRCLSocket.readUntilEndTag("CRCLStatus", bais);
			...
			try (ByteArrayInputStream bais = new ByteArrayInputStream((STATUS_XML + STATUS_XML).getBytes())) {
				String str = CRCLSocket.readUntilEndTag("CRCLStatus", bais);
				CRCLStatusType result = instance.stringToStatus(str, validate);
				assertEquals(1, result.getCommandStatus().getCommandID());
				assertEquals(1, result.getCommandStatus().getStatusID());
				str = CRCLSocket.readUntilEndTag("CRCLStatus", bais);
				result = instance.stringToStatus(str, validate);
				assertEquals(1, result.getCommandStatus().getCommandID());
				assertEquals(1, result.getCommandStatus().getStatusID());
				str = CRCLSocket.readUntilEndTag("CRCLStatus",
						new ByteArrayInputStream("  <CRCLStatus name=\"foo\" /> ".getBytes()));
				assertEquals(str, "<CRCLStatus name=\"foo\" />");
				str = CRCLSocket.readUntilEndTag("CRCLStatus", new ByteArrayInputStream(
						"  <CRCLStatus name=\"foo\" >  </CRCLStatus garbage-here  >  ".getBytes()));
	...
	public void testStringToCommand() throws Exception {
		...
		mttt.getWaypoint().add(CRCLPosemath.pose(CRCLPosemath.point(1, 2, 3), CRCLPosemath.vector(1, 0, 0),
				CRCLPosemath.vector(0, 0, 1)));
		mttt.getWaypoint().add(CRCLPosemath.pose(CRCLPosemath.point(3, 4, 4), CRCLPosemath.vector(1, 0, 0),
				CRCLPosemath.vector(0, 0, 1)));
